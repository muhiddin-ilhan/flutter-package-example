  ...
  void _prepareMask() {
    if (_separators.isEmpty) {
      for (var i = 0; i < mask.length; i++) {
        ...
        if (separatorChar != _anyCharMask && separatorChar != _onlyDigitMask && separatorChar != _onlyCharMask) {
          _separators.add(
            _Separator(
              value: separatorChar,
              indexInMask: i,
    ...
    for (var i = 0; i < splitMask.length; i++) {
      ...
      if (separator == null) {
        if (clearedValueAfter.length > index) {
          ...
          final maskOnCharMatcher = splitMask[i] == _onlyCharMask;
          var curChar = clearedValueAfter[index];
          if (maskOnDigitMatcher) {
            if (!isDigit(curChar, positiveOnly: true)) {
              break;
            }
          }else if (maskOnCharMatcher) {
            if (!isAlphabetic(curChar)) {
              break;
            }
          } else {
